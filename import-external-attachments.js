...
(function () {
  ...
  external_images_import_all = function (posts, next_post) {
    ...
    if (current_time > (import_images_start_time + 60000) && window.import_images_one_minute !== 'set') {
      target_message.prepend('<div>1 minute passed... Looking good!</div>');
      window.import_images_one_minute = 'set';
    }
    if (current_time > (import_images_start_time + 180000) && window.import_images_three_minute !== 'set') {
      target_message.prepend('<div>3 minutes passed... Still looking good!</div>');
      window.import_images_three_minute = 'set';
    }
    if (current_time > (import_images_start_time + 300000) && window.import_images_five_minute !== 'set') {
    ...
    if (current_time > (import_images_start_time + 600000) && window.import_images_ten_minute !== 'set') {
    ...
    if (current_time > (import_images_start_time + 1200000) && window.import_images_twenty_minute !== 'set') {
  ...
  //
  // var external_images_import_images = function () {
  //
